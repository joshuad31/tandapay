...
contract('TandaPayLedger|Functional', (accounts) => {
	...
	describe('Functional tests', function(){
		...
		await time.increase(time.duration.days(3)); // 33 DAY
			...
			await tandaPayLedger.processGroup(id, {from:cronAccount});
			var balArr = [(3+5/8)*premium, (5/8)*premium, (5/8)*premium, (5/8)*premium, (5/8)*premium, (5/8)*premium, (5/8)*premium, (5/8)*premium];
			var pcToBalCheck = [pc[0], pc[1], pc[2], pc[3], pc[4], pc[5], pc[6], pc[7]];
			await checkBalancesForThem(daiContract, pcToBalCheck, balArr);			
				// 2 period PHASE B
		...
		await time.increase(time.duration.days(3)); // 63 DAY
			...
			await tandaPayLedger.processGroup(id, {from:cronAccount});
			var balArr = [(3 + 5/8 + 3 + 2/8)*premium, (5/8 + 3 + 2/8)*premium, (5/8 + 2/8)*premium, (5/8 + 2/8)*premium, (5/8 + 2/8)*premium, (5/8 + 2/8)*premium, (5/8 + 2/8)*premium, (5/8 + 2/8)*premium];
			var pcToBalCheck = [pc[0], pc[1], pc[2], pc[3], pc[4], pc[5], pc[6], pc[7]];
			await checkBalancesForThem(daiContract, pcToBalCheck, balArr);
				// 3 period PHASE B
		...
		await time.increase(time.duration.days(30)); // 93 DAY
			...
			await tandaPayLedger.processGroup(id, {from:cronAccount});
			var balArr = [(3 + 5/8 + 3 + 2/8 + 8/3)*premium, (5/8 + 3 + 2/8 + 8/3)*premium, (5/8 + 2/8 + 8/3)*premium, (5/8 + 2/8)*premium, (5/8 + 2/8)*premium, (5/8 + 2/8)*premium, (5/8 + 2/8)*premium, (5/8 + 2/8)*premium];
			var pcToBalCheck = [pc[0], pc[1], pc[2], pc[3], pc[4], pc[5], pc[6], pc[7]];
			await checkBalancesForThem(daiContract, pcToBalCheck, balArr);
		...
		// 	daiContract = await DaiContract.new();
		// 	tandaPayLedger = await TandaPayLedger.new(daiContract.address, backend, cronAccount);		
		// 	tandaPayBase = await TandaPayBase.new();
		// });
	});
});
