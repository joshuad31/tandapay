...
contract TandaPayLedger {
	...
	function getNeededAmount(uint _groupID, address _phAddress) public view onlyValidGroupId(_groupID) returns(uint out) {
		out = 
	...
	function _getPremiumToPay(uint _groupID, address _phAddress) internal view onlyValidGroupId(_groupID) returns(uint) {
	...
	function _getOverpaymentToPay(uint _groupID, address _phAddress) internal view onlyValidGroupId(_groupID) returns(uint) {
		if(_isPolicyholderPremium(_groupID, _phAddress)) {
			return 0;
		}		
	...
	function _getLoanRepaymentToPay(uint _groupID, address _phAddress) internal view onlyValidGroupId(_groupID) returns(uint) {
		if(_isPolicyholderPremium(_groupID, _phAddress)) {
			return 0;
		}		
		uint MTR = 3; // TODO: ???
	...
	function addClaim(uint _groupID, address _claimantAddress) public onlyByBackend onlyValidGroupId(_groupID) returns(uint claimIndex) {
	...
	function removePolicyholderFromGroup(uint _groupID, uint _periodNumber, address _phAddress) public onlyByBackend onlyValidGroupId(_groupID) {
	...
	function commitPremium(uint _groupID, uint _amountDai) public onlyByPolicyholder(_groupID) onlyValidGroupId(_groupID) {
		uint period = getPeriodNumber(_groupID);
	...
	function addChangeSubgroupRequest(uint _groupID, uint _newSubgroupID) public onlyByPolicyholder(_groupID) onlyValidGroupId(_groupID) {
	...
	function finalizeClaims(uint _groupID, uint _periodIndex, bool _loyalist) public onlyByPolicyholder(_groupID) onlyValidGroupId(_groupID) {
	...
	function getGroupInfo2(uint _groupID) public view onlyValidGroupId(_groupID) returns(uint premiumsTotalDai, uint overpaymentTotalDai, uint loanRepaymentTotalDai) {
		for(uint i=0; i<groups[_groupID].policyholdersCount; i++) {
